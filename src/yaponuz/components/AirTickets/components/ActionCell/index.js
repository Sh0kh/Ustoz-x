...
import PreviewAirTicket from "../PreviewAirTicket";
      ...
      newSwal
        ...
        .then(async (result) => {
          ...
          if (result.isConfirmed) {
            ...
            const response = await Air.deleteAirticket(id, userId);
            loadingSwal.close();
  ...
  return (
    <SoftBox display="flex" alignItems="center">
      <SoftTypography variant="body1" color="secondary" sx={{ cursor: "pointer", lineHeight: 0 }}>
        <PreviewAirTicket id={myid} />
      </SoftTypography>
      <SoftBox mx={2}>
        <SoftTypography variant="body1" color="secondary" sx={{ cursor: "pointer", lineHeight: 0 }}>
          <UpdateAir myid={myid} itemme={itemme} refetch={refetch} />
        </SoftTypography>
      </SoftBox>
      <SoftTypography
        variant="body1"
