...
export default function UpdateQuestion() {
    ...
    const handleAudioChange = (e) => {
        ...
        if (file) {
            setAudio(URL.createObjectURL(file)); // Store the audio URL for preview
        }
    };
    ...
    const CreateQuestion = async () => {
        try {
            const NewData = {
                ...
                imageId: image ? 'image_name.jpg' : null, // Имя изображения (если есть)
                audioId: audio ? 'audio_name.mp3' : null, // Имя аудио (если есть)
                quizModuleId: ID
            };
