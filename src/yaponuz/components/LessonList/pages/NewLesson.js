...
function NewLesson() {
  ...
  useEffect(() => {
    if (isEditMode && location.state?.lessonData) {
      setLessonData(location.state.lessonData);
      setCourseId(location.state.lessonData.courseId);
    }
  ...
  const showAlert = (response) => {
    if (response.success) {
      Swal.fire(isEditMode ? "Updated!" : "Added!", response.message, "success").then(() =>
        navigateLesson()
      );
    } else {
      Swal.fire(
        isEditMode ? "Not Updated!" : "Not Added!",
  ...
  const handleSave = async () => {
    try {
      let response;
      if (isEditMode) {
        response = await Lesson.updateLesson({ id, ...lessonData });
      } else {
        response = await Lesson.createLesson(lessonData);
      }
      showAlert(response);
    } catch (err) {
      console.log(`Error from lesson ${isEditMode ? "update" : "create"}: `, err);
      Swal.fire("Error", `Failed to ${isEditMode ? "update" : "create"} lesson`, "error");
    }
  };
          ...
          <div className="flex h-full w-full px-8 items-center justify-between">
            ...
            <div>
              <SoftTypography variant="subtitle1" textTransform="uppercase" fontWeight="bold">
                &#9733;&#9733;&#9733; {isEditMode ? "Lesson Tahrirlash" : "Yangi Lesson Qo`shish"}{" "}
                &#9733;&#9733;&#9733;
              </SoftTypography>
            </div>
            <div>
