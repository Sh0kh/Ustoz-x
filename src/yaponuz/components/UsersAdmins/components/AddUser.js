...
import { UserAdmin } from "yaponuz/data/api";
...
export default function AddUser({ refetch }) {
  ...
  const handleSave = async () => {
    ...
    try {
      const response = await UserAdmin.createUser(data);
      showAlert(response);
    } catch (error) {
      console.log(error);
    }
  ...
  const showAlert = (response) => {
    ...
    if (response.success) {
      Swal.fire("Yangi admin qo‘shildi", response.message, "success").then(() => reload());
    } else {
